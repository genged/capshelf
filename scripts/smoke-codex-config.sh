...
(cd "$A/sub" && "${CLI[@]}" add codex-config/defaults --json > "$TMP/codex-add.json")
assert_fixed_contains 'model = "gpt-5"' "$A/.codex/config.toml"
assert_fixed_contains 'profile = "local"' "$A/.codex/config.toml"

CODEX_PATH="$(cd "$A/sub" && "${CLI[@]}" get-path codex-config/defaults)"
...
OUTPUT_PATH="$(cd "$A/sub" && "${CLI[@]}" get-path codex-config/defaults --output)"
...
(cd "$A/sub" && "${CLI[@]}" update codex-config/defaults --json > "$TMP/codex-update.json")
assert_fixed_contains 'model = "gpt-5.1"' "$A/.codex/config.toml"
assert_fixed_contains 'profile = "local"' "$A/.codex/config.toml"

perl -0pi -e 's/model = "gpt-5.1"\\n//' "$A/.codex/config.toml"
(cd "$A/sub" && "${CLI[@]}" revert codex-config/defaults --json > "$TMP/codex-revert.json")
assert_fixed_contains 'model = "gpt-5.1"' "$A/.codex/config.toml"

(cd "$A/sub" && "${CLI[@]}" rm codex-config/defaults --json > "$TMP/codex-rm.json")
assert_fixed_contains 'profile = "local"' "$A/.codex/config.toml"
assert_fixed_not_contains 'gpt-5.1' "$A/.codex/config.toml"

(cd "$A/sub" && "${CLI[@]}" add codex-config/defaults --json > "$TMP/codex-readd.json")
cat > "$DATA/codex/config/defaults/config.toml" <<'TOML'
model = "gpt-5.2"
sandbox = "workspace-write"
TOML
(cd "$A/sub" && "${CLI[@]}" promote codex-config/defaults -m 'codex defaults v3' --json > "$TMP/codex-promote.json")
