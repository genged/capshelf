...
mkdir -p "$CODEX/sub"
init_git_repo "$CODEX"
(cd "$CODEX/sub" && "${CLI[@]}" init --data ../../data --json > "$TMP/codex-init.json")
...
(cd "$CODEX/sub" && "${CLI[@]}" add skills/hello --json >/dev/null)
test -f "$CODEX/.agents/skills/hello/SKILL.md"
test -L "$CODEX/.claude/skills/hello"
test -f "$(cd "$CODEX/sub" && "${CLI[@]}" get-path skills/hello)/SKILL.md"
(cd "$CODEX/sub" && "${CLI[@]}" add skills/co-owned --json >/dev/null)

# --- claude-only mode: direct materialization, no .agents tree ---
CLAUDE="$TMP/claude-project"
mkdir -p "$CLAUDE/sub"
init_git_repo "$CLAUDE"
(cd "$CLAUDE/sub" && "${CLI[@]}" init --claude-only --data ../../data --json > "$TMP/claude-init.json")
...
(cd "$CLAUDE/sub" && "${CLI[@]}" add skills/hello --json >/dev/null)
...
(cd "$CODEX/sub" && "${CLI[@]}" status skills/hello --strict --json > "$TMP/codex-status-hello.json")
assert_contains 'acme/hello' "$TMP/codex-status-hello.json"
(cd "$CODEX/sub" && "${CLI[@]}" status skills/foreign-x --strict --json > "$TMP/codex-status-foreign-x.json")
...
if (cd "$CODEX/sub" && "${CLI[@]}" rm skills/hello > "$TMP/codex-rm-hello.txt" 2>&1); then
...
if (cd "$CODEX/sub" && "${CLI[@]}" add skills/foreign-x > "$TMP/codex-add-foreign-x.txt" 2>&1); then
...
(cd "$CODEX/sub" && "${CLI[@]}" status skills/co-owned --strict --json > "$TMP/codex-status-co-owned.json")
...
if (cd "$CODEX/sub" && "${CLI[@]}" rm skills/co-owned > "$TMP/codex-rm-co-owned.txt" 2>&1); then
...
for op in apply update revert promote rm; do
  if (cd "$CODEX/sub" && "${CLI[@]}" $op skills/foreign-x > "$TMP/codex-$op-foreign-x.txt" 2>&1); then
...
(cd "$SHADOW/sub" && "${CLI[@]}" status skills/hello > "$TMP/shadow-status-human.txt")
assert_contains 'external/  \(Personal Claude\)' "$TMP/shadow-status-human.txt"
HELLO_PATH="$(cd "$SHADOW/sub" && "${CLI[@]}" get-path skills/hello)"
test -f "$HELLO_PATH/SKILL.md"
printf '%s\n' 'local drift' >> "$SHADOW/.claude/skills/hello/SKILL.md"
printf '%s\n' 'stale' > "$SHADOW/.claude/skills/hello/stale.txt"
if (cd "$SHADOW/sub" && "${CLI[@]}" status skills/hello --strict > "$TMP/shadow-drift.txt" 2>&1); then
  echo "expected status --strict to fail on local drift"
  exit 1
fi
(cd "$SHADOW/sub" && "${CLI[@]}" apply skills/hello --dry-run --json > "$TMP/shadow-apply-dry-run.json")
...
(cd "$SHADOW/sub" && "${CLI[@]}" apply skills/hello --json > "$TMP/shadow-apply.json")
...
(cd "$SHADOW/sub" && "${CLI[@]}" apply skills/capshelf --json >/dev/null)
assert_contains '# capshelf' "$SHADOW/.claude/skills/capshelf/SKILL.md"
if (cd "$SHADOW/sub" && "${CLI[@]}" promote skills/capshelf > "$TMP/promote-system.txt" 2>&1); then
