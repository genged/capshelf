...
(cd "$A/sub" && "${CLI[@]}" add settings/security --json > "$TMP/settings-add.json")
...
(cd "$A/sub" && "${CLI[@]}" apply settings/security --json >/dev/null)
...
(cd "$A/sub" && "${CLI[@]}" status settings/security --json > "$TMP/settings-update-available.json")
...
(cd "$A/sub" && "${CLI[@]}" update settings/security --dry-run --json > "$TMP/settings-dry-run.json")
...
(cd "$A/sub" && "${CLI[@]}" update settings/security --json >/dev/null)
