...
(cd "$A/sub" && "${CLI[@]}" add mcp/github --json > "$TMP/mcp-add.json")
...
(cd "$A/sub" && "${CLI[@]}" status mcp/github --json > "$TMP/mcp-status.json")
assert_contains '"state": "update_available"' "$TMP/mcp-status.json"
(cd "$A/sub" && "${CLI[@]}" update mcp/github --dry-run --json > "$TMP/mcp-dry-run.json")
...
(cd "$A/sub" && "${CLI[@]}" update mcp/github --json > "$TMP/mcp-update.json")
...
if (cd "$A/sub" && "${CLI[@]}" status mcp/github --strict >/dev/null 2>&1); then
  echo "expected strict status to fail on output drift"
  exit 1
fi
(cd "$A/sub" && "${CLI[@]}" apply mcp/github --json > "$TMP/mcp-apply.json")
assert_fixed_contains 'github-mcp-v2' "$A/.mcp.json"
...
enabled = true
TOML
(cd "$A/sub" && "${CLI[@]}" promote mcp/github -m 'github mcp v3' --json > "$TMP/mcp-promote.json")
